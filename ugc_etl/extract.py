...
class Extract:
    @backoff.on_exception(
        backoff.expo, [NoBrokersAvailable, ValueError], on_backoff=backoff_hdlr
    )
    ...
    def run(self):
        ...
        for message in self.extract:
            if message.topic == TOPIC_BOOKMARKS:
                data = self.transform.get_bookmark_data(
