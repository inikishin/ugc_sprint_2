        ...
        **dsl, cursor_factory=DictCursor
        ...
        for table_name in sqlite_data.keys():
            for sqlite_rec in sqlite_data[table_name]:
                pg_record = postgres_saver.get_table_record_by_id(
                    table_name, sqlite_rec.get("id")
                )
                if pg_record:
                    for field in pg_record.keys():
                        ...
                        if sq_field != pg_field:
                            checked = False
                            logger.debug(
                                "Table: {0} | field not equal {1} | record: {2} || {3}".format(
                                    table_name, field, sqlite_rec, pg_record
                                )
                            )
                else:
                    logger.debug(
                        "Table: {0} | Record {1} not found in pg base".format(
                            table_name, sqlite_rec
                        )
                    )
                    checked = False
