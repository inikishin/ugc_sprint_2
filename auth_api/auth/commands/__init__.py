...
def create_super_user():
    ...
    first_name = click.prompt("First name", type=str, prompt_suffix=prompt_suffix)
    last_name = click.prompt("Last name", type=str, prompt_suffix=prompt_suffix)

    while True:
        password = click.prompt(
            "Password", type=str, hide_input=True, prompt_suffix=prompt_suffix
        )
    ...
    if click.confirm("Do you want to create a superuser?"):
        ...
        if role_service.is_role_exists("admin") is False:
            role_service.create_role({"name": "admin", "description": "Superuser role"})
