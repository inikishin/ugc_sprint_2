...
def get_authorize_url(provider: str):
    ...
    return {"authorize_url": oauth_service.get_authorize_url()}
...
def receive_verification_code(provider: str):
    ...
    user_info = oauth_service.get_user_info(access_token=token_data.get("access_token"))
    ...
    user_service.save_user_oauth_refresh_token(
        user.id, PROVIDER, token_data.get("refresh_token"), token_data.get("expires_in")
    )
...
def get_user_info(provider: str):
    ...
    refresh_token = user_service.get_user_oauth_refresh_token(current_user.id, PROVIDER)

    data = oauth_service.refresh_token(refresh_token=refresh_token)

    user_service.save_user_oauth_refresh_token(
        current_user.id, PROVIDER, data.get("refresh_token"), data.get("expires_in")
    )
    user_info = oauth_service.get_user_info(access_token=data.get("access_token"))
