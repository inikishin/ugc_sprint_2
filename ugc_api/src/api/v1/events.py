...
async def send_rating(
    movie_rating: MovieRating, event_service: EventService = Depends(get_event_service)
):
    """В теле запроса отправляются данные пользовательского рейтинга для фильма."""
    event_service.send_rating(
...
async def send_history(
    ...
    event_service.send_history(
        movie_history.user_id, movie_history.movie_id, movie_history.viewed
    )
    return SuccessResponse()
...
async def send_last_view_time(
    movie_last_view_time: MovieLastViewTime,
    event_service: EventService = Depends(get_event_service),
):
    event_service.send_last_view_time(
        movie_last_view_time.user_id,
