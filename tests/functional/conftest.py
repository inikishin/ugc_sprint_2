...
SERVICE_URL = "http://{host}:{port}".format(
    host=os.getenv("API_HOST", "127.0.0.1"), port=os.getenv("API_PORT", 8000)
)
...
async def es_client():
    ...
    client = AsyncElasticsearch(hosts=[elastic_conn_string])
    yield client
    await client.close()
...
def auth_with_superuser(auth_runner):
    auth_runner.invoke(
        ...
        input="\n".join(["test@email.com", "first", "admin", "test", "test", "y"]),
    )
...
def auth_get_headers(auth_client, auth_with_superuser):
    response_login = auth_client.post(
        "/auth/login", json={"login": "test@email.com", "password": "test"}
    )

    return {"Authorization": "Bearer " + response_login.json["access_token"]}
...
def auth_get_headers_for_refresh(auth_client, auth_with_superuser):
    ...
    return {"Authorization": "Bearer " + response_login.json["refresh_token"]}
